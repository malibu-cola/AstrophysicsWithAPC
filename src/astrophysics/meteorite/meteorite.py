...
class MeteoriteProperty:
    ...
    def du_dt(self) -> float:
        ...
        ret = (
            -self.k1
            * self.density_atmosphere
            * self.velocity.speed
            * self.velocity.u
            * np.exp(-1 / 3 * np.log(self.mass))
        )
        # print("du_dt: ",ret)
        return ret
    ...
    def dmass_dt(self) -> float:
        ...
        ret = (
            -self.k2
            * self.density_atmosphere
            * (self.velocity.speed**3)
            * np.exp(2 / 3 * np.log(self.mass))
        )
        # print("dmass_dt: ",ret)
        return ret
...
class Meteorite:
    ...
    def __init__(
        self, y0: float, u: float, v: float, m0: float, k1: float, k2: float, tau: float
    ):
        initial_property = MeteoriteProperty(
            0, _Coordinate(0, y0), _Velocity(u, v), m0, k1, k2, tau
        )
    ...
    def calc(self):
        ...
        while self._is_continue:
            ...
            meteo_for_calc = MeteoriteProperty(
                t,
                _Coordinate(x1, y1),
                _Velocity(u1, v1),
                m1,
            ...
            x = meteo.coord.x + 0.5 * self.dt * (
                meteo.velocity.u + meteo_for_calc.velocity.u
            )
            y = meteo.coord.y + 0.5 * self.dt * (
                meteo.velocity.v + meteo_for_calc.velocity.v
            )
            ...
            next_meteo = MeteoriteProperty(
                t, _Coordinate(x, y), _Velocity(u, v), m, meteo.k1, meteo.k2, meteo.tau
            )
    ...
    def plot(self):
        ...
        fig.suptitle(
            "Meteorite Properties: y0 = {:.1f} km, u = {:.1f} km/s, v = {:.1f} km/s, m0 = {:.1f} g".format(
                initial_condition["y"] / 1e5,
                initial_condition["u"] / 1e5,
                initial_condition["v"] / 1e5,
