...
class Coordinate:
    ...
    def to_solar_system(self, comet_nuclear_coord: "Coordinate") -> "Coordinate":
        """座標系を太陽系に変換する"""
        ...
        match self.system:
            ...
            case CoordinateSystem.COMET_SYSTEM:
                ...
                return Coordinate(CoordinateSystem.SOLAR_SYSTEM, x_prime, y_prime)

    def to_comet_system(self, comet_nuclear_coord: "Coordinate") -> "Coordinate":
        """座標系を彗星系に変換する"""
        match comet_nuclear_coord.system:
            case CoordinateSystem.SOLAR_SYSTEM:
                x = comet_nuclear_coord.x
        ...
        match self.system:
            case CoordinateSystem.SOLAR_SYSTEM:
                ...
                return Coordinate(CoordinateSystem.COMET_SYSTEM, s, t)
            case CoordinateSystem.COMET_SYSTEM:
                return self
...
class Comet:
    ...
    def nuclear_coord(self, nu: float | np.ndarray) -> Coordinate:
        """軌道上の座標(x, y)を計算する"""
        x = self.radius_of_orbit(nu) * np.cos(nu)
        y = self.radius_of_orbit(nu) * np.sin(nu)
        return Coordinate(CoordinateSystem.SOLAR_SYSTEM, x, y)
    ...
    def calculate_syndyname(
        ...
        """シンダイン曲線の座標を計算する"""
        if (isinstance(s, float) and s < 0) or (isinstance(s, np.ndarray) and (s < 0).any()):
    ...
    def _draw_syndyname(self, nu: float):
        ...
        for escape_direction in escape_directions:
            particle_coord_s = np.linspace(0.05, 0.5, 100)  # 彗星座標系における粒子の座標s
        
            particle_coord_t = self.calculate_syndyname(
                particle_coord_s, escape_direction
            )  # 彗星座標系における粒子の座標t

            comet_particle_coord = Coordinate(
            ...
            plt.plot(comet_particle_coord.x, comet_particle_coord.y, "o", color="blue", markersize=2)
